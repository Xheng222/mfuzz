...
设网络集合 F = {F_1, ..., F_n}，目标模型 F_j，共识标签 c：
    ...
    obj_1(x) = Σ_{k≠j} F_k(x)[c] - λ1 · F_j(x)[c]
...
def differential_objective(
    probs: dict[str, Tensor], target: str, c: Tensor, lambda1: float = 1.0
) -> Tensor:
    """逐样本 obj_1。
    ...
    return ref_term - lambda1 * target_term
