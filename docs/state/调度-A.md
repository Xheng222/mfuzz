...
| 作业 | 流 | 位置 | 状态 | 预计 |
|------|----|------|------|------|
| regen_full 全量四模型数据生成（t_cov=0.95 重标版） | 实验 | 服务器 GPU0，启动器 PID 403065/403068、worker PID 403071 | 运行中、构建阶段；CNCov_0≈0.31 确认待定（本地定时器 bdd54it2o ~18min 后查日志） | 数小时~十几小时，明天 pull |
