...
- 实验：✓ loc generated 全量跑完并拉回。fcos 干净正例（责任 head.regression_head 修掉约 40% loc、对照 random/bottom 全 0），retina 一致反例（责任头修不动、与 retina loc 归因偏 FPN 一致），合起来"修复跟着归因走"。**按用户意愿，这条保留为弱的、接近本质的假设——闭环方式（fuzz→定位→定向微调修复）有效——不复核、不升 [结论]、有意保持弱叙述（不声称统计强度与普遍性，N 偏小 B 33/36）。** 产物 generated_loc_{fcos,retinanet}/data/{result.json,frontier.png}。可选跟进（不阻塞、按需）：retina loc 责任结构按归因改成 FPN 指认层再跑、cls 负向案例 2×2 叙述。cls 不 densify；虚检/漏检免训练抑制因果结论保留。对照纪律与判据见 docs/paper_plan/定位与修复验证框架.md。
